...
print_every = 1
plot_every = 1
mask_frac=5  ###############masking fraction , 5 and 10 tested
n_channels=3 
image_size = 512
...
slice_gap = 0
reg_weight_mse = 0.2
reg_weight_mask_mse = 0.2
reg_weight_qloss = 0.8
reg_weight_ssim = -0.1
...
fig_dir = ################### path to directory to save input/output figureds after evry 10 batches
if os.path.exists(fig_dir)==False:
    os.mkdir(fig_dir)
    
device_ids = [0, 1] ################### device ids are available gpu ids
metric_name = 'loss'
lr = 1e-3
...
early_stop=False
prev_val_loss = 1e10
model_path=None###############provide path if model to be started from some checkpoint
offset = 0

    
# DATA
df_train = #provide CSV data file#
df_val = #provide CSV data file#
print('data read .....')
